...
**新增** 本版本进一步集成智能模型路由功能，通过基于任务特征、优先级和阶段的动态模型选择，实现更精细化的成本优化。模型路由允许根据任务的具体需求自动选择最合适的模型，从而在保证质量的同时最大化成本效益。

## 项目结构
围绕成本优化的关键模块分布如下：
- 示例与模式：examples/patterns/cost-tiered-pipeline.ts 展示了四阶段流水线与分层定价估算
- 模型路由：docs/model-routing.md 提供智能模型选择的配置与使用指南
...
- **智能模型路由**：基于任务角色、优先级和执行阶段的动态模型选择机制，允许在编排层面对模型进行智能切换，实现更精细化的成本控制。
...
User->>OM : 配置默认参数与回调
OM->>Team : 创建团队(含共享内存)
Team->>Router : 应用模型路由规则
Router->>Agent : 动态选择最优模型
...
- [trace.ts:12-27](file://src/utils/trace.ts#L12-L27)
- [types.ts:154-187](file://src/types.ts#L154-L187)
- [orchestrator.ts:687](file://src/orchestrator/orchestrator.ts#L687)
...
### 组件B：智能模型路由
- 设计理念
  - 基于任务特征的动态模型选择：根据任务角色、优先级和执行阶段自动匹配最优模型。
  - 规则驱动的路由机制：支持条件判断和优先级排序，确保第一个匹配的规则生效。
  - 编排层集成：在团队编排阶段应用路由规则，实现端到端的成本优化。
- 关键实现要点
  - ModelRoutingRule 接口定义了路由规则的结构，包括模型选择条件和目标模型配置。
  - routeMatches 函数实现了规则匹配逻辑，支持相位(phase)、代理(agent)和自定义属性的匹配。
  - Orchestrator 集成点：在多个关键执行点应用模型路由，包括协调器、合成器和短路阶段。
- 配置建议
  - 优先级设置：将最严格的条件放在前面，确保精确匹配优先于宽松匹配。
  - 条件组合：合理组合任务角色、优先级和阶段条件，避免规则冲突。
  - 成本控制：为高成本模型设置使用上限，防止意外的高成本调用。

```mermaid
flowchart TD
Start(["模型路由开始"]) --> CheckRules["检查路由规则列表"]
CheckRules --> Match{"规则匹配?"}
Match --> |是| SelectModel["选择匹配的模型配置"]
Match --> |否| NextRule["检查下一个规则"]
NextRule --> CheckRules
SelectModel --> Apply["应用到当前执行阶段"]
Apply --> End(["路由完成"])
```

**图表来源**
- [types.ts:736](file://src/types.ts#L736)
- [types.ts:743](file://src/types.ts#L743)
- [orchestrator.ts:687](file://src/orchestrator/orchestrator.ts#L687)

**章节来源**
- [types.ts:705](file://src/types.ts#L705)
- [types.ts:735](file://src/types.ts#L735)
- [types.ts:741](file://src/types.ts#L741)
- [types.ts:775](file://src/types.ts#L775)
- [types.ts:813](file://src/types.ts#L813)
- [types.ts:975](file://src/types.ts#L975)
- [types.ts:977](file://src/types.ts#L977)
- [orchestrator.ts:618](file://src/orchestrator/orchestrator.ts#L618)
- [orchestrator.ts:687](file://src/orchestrator/orchestrator.ts#L687)
...
- **模型路由集成**
  - model-routing.md 与 orchestrator.ts 深度集成，通过 ModelRoutingPolicy 和 routeMatches 实现智能模型选择。
  - 路由规则在编排器的多个执行阶段生效，包括协调器、合成器和短路阶段。
- 执行引擎依赖可观测性与预算类型
...
- **智能模型选择**
  - 通过模型路由根据任务特征自动选择最优模型，避免手动配置的不确定性。
  - 支持基于任务角色、优先级和阶段的精细化控制，确保关键任务使用高性能模型。
- 精确处理与中间态优化
...
- **模型路由问题**
  - 现象：模型未按预期切换或路由规则未生效。
  - 处理：检查 ModelRoutingRule 的条件设置，确保规则按优先级正确排序；验证 routeMatches 的匹配逻辑；确认 Orchestrator 中的路由应用点。
- 工具调用与预算顺序
...
## 结论
通过"成本分层流水线"与"智能模型路由"的双重优化机制，Open Multi-Agent 框架能够在不同预算约束下灵活调整模型层级与处理策略，实现成本与质量的动态平衡。智能模型路由进一步增强了系统的自动化程度，通过基于任务特征的动态模型选择，确保关键任务获得足够的计算资源，同时在非关键环节使用低成本模型降低成本。配合预算控制、可观测性与令牌估算，系统能够稳定地在复杂任务中控制开销并提升可运维性。建议在实际应用中：
- 以任务复杂度为依据划分层级，将"确定性工作"下沉至低成本模型；
- **利用智能模型路由自动选择最优模型，减少人工配置的不确定性；**
...
- **智能模型路由**
  - 配置文档：docs/model-routing.md
  - 关键接口：ModelRoutingRule、ModelRoutingPolicy、routeMatches
  - 应用点：orchestrator.ts 中的多个执行阶段
- 预算控制
