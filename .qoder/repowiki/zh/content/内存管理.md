# 内存管理

<cite>
**本文引用的文件**
- [src/memory/store.ts](file://src/memory/store.ts)
- [src/memory/shared.ts](file://src/memory/shared.ts)
- [src/memory/checkpoint.ts](file://packages/core/src/memory/checkpoint.ts)
- [src/types.ts](file://src/types.ts)
- [docs/shared-memory.md](file://docs/shared-memory.md)
- [docs/checkpoint.md](file://docs/checkpoint.md)
- [examples/integrations/with-engram/engram-store.ts](file://examples/integrations/with-engram/engram-store.ts)
- [examples/integrations/with-engram/engram-toolkit.ts](file://examples/integrations/with-engram/engram-toolkit.ts)
- [examples/integrations/with-engram/team-research.ts](file://examples/integrations/with-engram/team-research.ts)
- [examples/integrations/with-engram/research-team.ts](file://examples/integrations/with-engram/research-team.ts)
- [tests/shared-memory.test.ts](file://tests/shared-memory.test.ts)
- [packages/core/tests/checkpoint.test.ts](file://packages/core/tests/checkpoint.test.ts)
</cite>

## 更新摘要
**所做更改**
- 新增检查点恢复系统章节，涵盖持久化存储选项和恢复机制
- 更新架构总览图，包含检查点管理器组件
- 扩展性能与容量规划，增加检查点存储考虑
- 新增检查点最佳实践和故障排查指南
- 更新配置示例，包含检查点配置选项

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [组件详解](#组件详解)
6. [检查点恢复系统](#检查点恢复系统)
7. [依赖关系分析](#依赖关系分析)
8. [性能与容量规划](#性能与容量规划)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录：配置与集成示例](#附录配置与集成示例)

## 简介
本文件系统性阐述 Open Multi-Agent 框架的内存管理机制，重点覆盖：
- 默认进程内键值存储实现与数据模型
- 共享内存（团队级）的设计与使用方式
- 检查点恢复系统，支持长期任务的工作流持久化和崩溃恢复
- 如何实现自定义 MemoryStore 接口以对接 Redis、PostgreSQL 或其他后端
- 共享内存的 TTL 过期、清理与持久化策略
- 检点存储的优化策略和最佳实践
- 性能考量与容量规划建议
- 完整配置示例与集成指南

## 项目结构
内存管理相关代码主要位于以下模块：
- 类型与接口定义：src/types.ts
- 默认内存实现：src/memory/store.ts
- 团队共享内存层：src/memory/shared.ts
- 检查点管理器：packages/core/src/memory/checkpoint.ts
- 文档说明：docs/shared-memory.md 和 docs/checkpoint.md
- 示例：examples/integrations/with-engram/*
- 测试：tests/shared-memory.test.ts 和 packages/core/tests/checkpoint.test.ts

```mermaid
graph TB
subgraph "类型与接口"
T["src/types.ts<br/>MemoryStore/MemoryEntry 接口"]
end
subgraph "内存实现"
IS["src/memory/store.ts<br/>InMemoryStore 默认实现"]
SM["src/memory/shared.ts<br/>SharedMemory 团队共享层"]
CP["packages/core/src/memory/checkpoint.ts<br/>Checkpoint 检查点管理器"]
end
subgraph "示例与工具"
ES["examples/integrations/with-engram/engram-store.ts<br/>EngramMemoryStore"]
ET["examples/integrations/with-engram/engram-toolkit.ts<br/>Engram 工具集"]
ER["examples/integrations/with-engram/research-team.ts<br/>示例脚本"]
ERT["examples/integrations/with-engram/team-research.ts<br/>Orchestrator 集成"]
end
subgraph "测试"
STS["tests/shared-memory.test.ts<br/>行为验证"]
CTS["packages/core/tests/checkpoint.test.ts<br/>检查点功能测试"]
end
T --> IS
T --> SM
T --> CP
IS --> SM
IS --> CP
SM --> CP
ES --> SM
ET --> ER
ET --> ERT
STS --> SM
CTS --> CP
```

**图表来源**
- [src/types.ts:992-1106](file://src/types.ts#L992-L1106)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:1-105](file://packages/core/src/memory/checkpoint.ts#L1-L105)
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)
- [examples/integrations/with-engram/engram-toolkit.ts:34-194](file://examples/integrations/with-engram/engram-toolkit.ts#L34-L194)
- [examples/integrations/with-engram/research-team.ts:1-226](file://examples/integrations/with-engram/research-team.ts#L1-L226)
- [examples/integrations/with-engram/team-research.ts:178-198](file://examples/integrations/with-engram/team-research.ts#L178-L198)
- [tests/shared-memory.test.ts:1-301](file://tests/shared-memory.test.ts#L1-L301)
- [packages/core/tests/checkpoint.test.ts:1-578](file://packages/core/tests/checkpoint.test.ts#L1-L578)

**章节来源**
- [src/types.ts:992-1106](file://src/types.ts#L992-L1106)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:1-105](file://packages/core/src/memory/checkpoint.ts#L1-L105)
- [docs/shared-memory.md:1-28](file://docs/shared-memory.md#L1-L28)
- [docs/checkpoint.md:1-135](file://docs/checkpoint.md#L1-L135)
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)
- [examples/integrations/with-engram/engram-toolkit.ts:34-194](file://examples/integrations/with-engram/engram-toolkit.ts#L34-L194)
- [examples/integrations/with-engram/research-team.ts:1-226](file://examples/integrations/with-engram/research-team.ts#L1-L226)
- [examples/integrations/with-engram/team-research.ts:178-198](file://examples/integrations/with-engram/team-research.ts#L178-L198)
- [tests/shared-memory.test.ts:1-301](file://tests/shared-memory.test.ts#L1-L301)
- [packages/core/tests/checkpoint.test.ts:1-578](file://packages/core/tests/checkpoint.test.ts#L1-L578)

## 核心组件
- MemoryStore 接口：统一的键值存储抽象，支持可选的过期写入能力。
- MemoryEntry 数据模型：记录键、值、元数据与创建时间；可选的过期回合数。
- InMemoryStore：默认的进程内实现，基于 Map 存储，提供基本 CRUD 与简单检索。
- SharedMemory：团队共享层，负责命名空间隔离、TTL 过期过滤、摘要生成等。
- Checkpoint：检查点管理器，提供持久化存储和恢复机制，支持多种存储后端。

**章节来源**
- [src/types.ts:974-1011](file://src/types.ts#L974-L1011)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:35-105](file://packages/core/src/memory/checkpoint.ts#L35-L105)

## 架构总览
SharedMemory 通过 MemoryStore 抽象与具体后端解耦。默认使用 InMemoryStore，也可注入自定义实现（如 EngramMemoryStore）。Checkpoint 系统独立于共享内存，提供持久化存储和恢复能力。SharedMemory 负责：
- 命名空间写入：将 agentName 作为前缀拼接为完整键
- TTL 写入：可选调用 setWithExpiry，由后端或 SharedMemory 自身进行过期判断
- 读取过滤：在返回前剔除已过期条目
- 列表与摘要：按 agent 分组输出人类可读摘要

Checkpoint 系统负责：
- 检查点持久化：在任务完成后保存检查点快照
- 恢复机制：从检查点重建任务队列和共享内存状态
- 键空间管理：使用预留的检查点键空间避免与共享内存冲突

```mermaid
classDiagram
class MemoryStore {
+get(key) Promise<MemoryEntry|null>
+set(key, value, metadata) Promise<void>
+setWithExpiry?(key, value, expiresAtTurn, metadata) Promise<void>
+list() Promise<MemoryEntry[]>
+delete(key) Promise<void>
+clear() Promise<void>
}
class MemoryEntry {
+key : string
+value : string
+metadata? : Record<string, unknown>
+createdAt : Date
+expiresAtTurn? : number
}
class InMemoryStore {
-data : Map<string, MemoryEntry>
+get(key)
+set(key, value, metadata)
+setWithExpiry(key, value, expiresAtTurn, metadata)
+list()
+delete(key)
+clear()
+search(query)
+size
+has(key)
}
class SharedMemory {
-store : MemoryStore
-turnCount : number
+advanceTurn()
+getTurnCount()
+write(agentName, key, value, metadata)
+writeExpiring(agentName, key, value, ttlTurns, metadata)
+read(key)
+listAll()
+listByAgent(agentName)
+getSummary(filter?)
+getStore()
}
class Checkpoint {
-key : string
-runId : string
+save(snapshot)
+loadLatest()
+load()
+delete()
}
MemoryStore <|.. InMemoryStore
SharedMemory --> MemoryStore : "依赖"
SharedMemory --> MemoryEntry : "使用"
Checkpoint --> MemoryStore : "依赖"
```

**图表来源**
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)
- [src/types.ts:974-987](file://src/types.ts#L974-L987)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:35-105](file://packages/core/src/memory/checkpoint.ts#L35-L105)

## 组件详解

### MemoryStore 接口与 MemoryEntry 数据模型
- MemoryStore：定义 get/set/list/delete/clear；setWithExpiry 为可选，用于回合制 TTL。
- MemoryEntry：统一的存储单元，包含键、值、元数据、创建时间与可选的 expiresAtTurn。

**章节来源**
- [src/types.ts:974-1011](file://src/types.ts#L974-L1011)

### InMemoryStore 默认实现
- 存储介质：内存 Map，适合单进程与测试场景。
- 行为特性：
  - get/set 保持 createdAt 不变，便于检测首次写入时间
  - setWithExpiry 将过期回合数写入条目，供 SharedMemory 过滤
  - list 返回插入顺序快照
  - delete 清除指定键
  - clear 清空所有
  - search 提供简单的子串匹配（大小写不敏感），线性扫描
  - size/has 提供便捷查询

**章节来源**
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)

### SharedMemory 团队共享层
- 命名空间：写入时自动拼接为 agentName/key，避免冲突并保留归属
- TTL 支持：writeExpiring 会调用 setWithExpiry（若后端实现），否则降级为普通 set
- 读取过滤：read 在返回前检查过期，不过期删除由后端自行处理
- 列表与摘要：listAll/listByAgent 过滤过期项；getSummary 生成人类可读摘要，按 agent 分组
- 轮次计数：advanceTurn 递增内部计数器，驱动 TTL 到期判断

```mermaid
sequenceDiagram
participant Agent as "Agent"
participant SM as "SharedMemory"
participant Store as "MemoryStore"
Agent->>SM : write(agentName, key, value, metadata)
SM->>SM : namespaceKey(agentName, key)
SM->>Store : set(namespacedKey, value, {agent, ...metadata})
Store-->>SM : ok
Agent->>SM : read("agentName/key")
SM->>Store : get("agentName/key")
Store-->>SM : MemoryEntry | null
SM->>SM : isExpired(entry)?
alt 已过期
SM-->>Agent : null
else 未过期
SM-->>Agent : MemoryEntry
end
```

**图表来源**
- [src/memory/shared.ts:124-205](file://src/memory/shared.ts#L124-L205)
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)

**章节来源**
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)

### 自定义 MemoryStore 实现指南
- 必备方法：get/set/list/delete/clear
- 可选方法：setWithExpiry（若实现 TTL）
- 关键约束：
  - keys 为字符串；values 为字符串（结构化数据需序列化）
  - 元数据透传给调用方（SharedMemory 会追加 agent 标记）
  - 若未实现 setWithExpiry，SharedMemory 会降级为普通写入，不保证 TTL
- 示例参考：EngramMemoryStore 通过 REST API 实现 MemoryStore 接口，将 scope 映射为 key，使用 operation 控制更新/删除语义。

**章节来源**
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)

### 共享内存的使用与配置
- 启用团队共享内存：在团队配置中设置 sharedMemory=true 使用默认 InMemoryStore
- 注入自定义存储：通过 sharedMemoryStore 传入自定义 MemoryStore 实例
- 当两者同时提供时，sharedMemoryStore 优先
- 文档示例展示了上述两种模式

**章节来源**
- [docs/shared-memory.md:1-28](file://docs/shared-memory.md#L1-L28)
- [src/memory/shared.ts:77-85](file://src/memory/shared.ts#L77-L85)

### 示例：Engram 集成
- EngramMemoryStore：实现 MemoryStore，将 scope 映射为 key，使用 operation 控制更新/删除
- EngramToolkit：注册四个工具（engram_commit、engram_query、engram_conflicts、engram_resolve）
- 示例脚本：演示如何在 runTeam 中注入 EngramMemoryStore，实现跨任务共享记忆

**章节来源**
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)
- [examples/integrations/with-engram/engram-toolkit.ts:34-194](file://examples/integrations/with-engram/engram-toolkit.ts#L34-L194)
- [examples/integrations/with-engram/team-research.ts:178-198](file://examples/integrations/with-engram/team-research.ts#L178-L198)
- [examples/integrations/with-engram/research-team.ts:1-226](file://examples/integrations/with-engram/research-team.ts#L1-L226)

### 测试与行为验证
- 命名空间隔离：不同 agent 的同名 key 不冲突
- 元数据合并：写入时会合并 agent 标记
- 摘要生成：按 agent 分组输出，长值截断
- 形状校验：构造函数对传入的 MemoryStore 进行结构检查，防止错误对象进入
- 团队配置：sharedMemory=true 仅时默认 InMemoryStore；注入无效对象会抛错

**章节来源**
- [tests/shared-memory.test.ts:1-301](file://tests/shared-memory.test.ts#L1-L301)

## 检查点恢复系统

### 概述
检查点恢复系统允许长期运行的任务工作流在崩溃、中止或进程重启后继续执行。该系统完全基于现有的 MemoryStore 接口，因此持有共享内存的相同存储后端（内存、Redis、Postgres 或自定义后端）也用于检查点持久化——无需额外的存储层。

### 核心组件
- Checkpoint 类：提供检查点的保存、加载和删除功能
- 检查点键空间：使用预留的 `__oma_checkpoint__/` 命名空间避免与共享内存冲突
- 检查点快照：包含任务队列状态、共享内存和已完成任务结果的 JSON 序列化数据

### 检查点存储策略
- **共享内存优化**：当检查点存储与团队共享内存存储相同时，只保存轮次计数而不重复嵌入共享内存条目
- **分离存储**：当使用独立的检查点存储时，会嵌入完整的共享内存快照
- **键空间隔离**：检查点键使用预留前缀，确保与共享内存条目不会冲突

### 恢复机制
- **任务级粒度**：恢复发生在任务边界，不支持任务中间状态的恢复
- **最佳努力**：检查点写入失败不会影响主运行，会在下个完成的任务时重试
- **合成重放**：runTeam 恢复会重新运行协调器合成，确保最终答案的一致性

```mermaid
sequenceDiagram
participant Runner as "运行器"
participant Checkpoint as "检查点管理器"
participant Store as "MemoryStore"
participant Queue as "任务队列"
Runner->>Checkpoint : save(snapshot)
Checkpoint->>Store : set(checkpointKey, JSON(snapshot))
Store-->>Checkpoint : ok
Runner->>Checkpoint : loadLatest()
Checkpoint->>Store : get(checkpointKey)
Store-->>Checkpoint : JSON(snapshot)
Checkpoint->>Queue : fromSnapshot(snapshot.queue)
Queue-->>Runner : 恢复的任务队列
```

**图表来源**
- [packages/core/src/memory/checkpoint.ts:47-89](file://packages/core/src/memory/checkpoint.ts#L47-L89)
- [packages/core/tests/checkpoint.test.ts:167-187](file://packages/core/tests/checkpoint.test.ts#L167-L187)

### 配置选项
- `enabled`：启用/禁用检查点功能
- `store`：检查点存储的 MemoryStore 实例
- `runId`：逻辑运行 ID，派生每个运行的检查点键
- `key`：精确的存储键，优先于 runId

### 使用示例
- **启用检查点**：在 runTasks 调用中传递 `{ checkpoint: { store } }`
- **全局启用**：通过 OrchestratorConfig 设置默认检查点配置
- **恢复运行**：使用 restore() 方法从检查点恢复

**章节来源**
- [docs/checkpoint.md:1-135](file://docs/checkpoint.md#L1-L135)
- [packages/core/src/memory/checkpoint.ts:35-105](file://packages/core/src/memory/checkpoint.ts#L35-L105)
- [packages/core/tests/checkpoint.test.ts:190-395](file://packages/core/tests/checkpoint.test.ts#L190-L395)

## 依赖关系分析
- SharedMemory 依赖 MemoryStore 接口，从而与具体后端解耦
- Checkpoint 独立于 SharedMemory，但都依赖 MemoryStore 接口
- InMemoryStore 与 EngramMemoryStore 都实现 MemoryStore
- SharedMemory 依赖 MemoryEntry 数据模型
- 检查点系统与共享内存系统相互独立但可以共享存储后端

```mermaid
graph LR
Types["src/types.ts<br/>接口/类型"] --> Impl1["src/memory/store.ts<br/>InMemoryStore"]
Types --> Impl2["examples/integrations/with-engram/engram-store.ts<br/>EngramMemoryStore"]
Types --> SM["src/memory/shared.ts<br/>SharedMemory"]
Types --> CP["packages/core/src/memory/checkpoint.ts<br/>Checkpoint"]
Impl1 --> SM
Impl2 --> SM
Impl1 --> CP
Impl2 --> CP
SM --> CP
```

**图表来源**
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:1-105](file://packages/core/src/memory/checkpoint.ts#L1-L105)

**章节来源**
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)
- [src/memory/store.ts:31-149](file://src/memory/store.ts#L31-L149)
- [examples/integrations/with-engram/engram-store.ts:48-188](file://examples/integrations/with-engram/engram-store.ts#L48-L188)
- [src/memory/shared.ts:55-335](file://src/memory/shared.ts#L55-L335)
- [packages/core/src/memory/checkpoint.ts:1-105](file://packages/core/src/memory/checkpoint.ts#L1-L105)

## 性能与容量规划
- 默认 InMemoryStore
  - 优点：低延迟、易部署、适合单进程与测试
  - 缺点：无持久化、无 TTL 清理、无跨进程共享
  - 复杂度：get/set/list/delete/clear 均为 O(1)/O(n) 线性扫描（search/list）
  - 建议：小规模团队、短期实验；注意内存增长与定期清理
  - **新增**：检查点存储建议使用独立的存储实例，避免与共享内存竞争
- EngramMemoryStore
  - 优点：持久化、跨进程共享、冲突解决与审计
  - 缺点：网络开销、依赖外部服务
  - 建议：生产环境首选；关注 API 限流与重试策略
  - **新增**：检查点存储可与共享内存存储相同，享受共享内存优化
- Redis/PostgreSQL 等自定义后端
  - Redis：适合高吞吐、低延迟键值场景；可利用原生存储 TTL 与过期事件
  - PostgreSQL：适合需要复杂查询、审计与事务一致性场景；可结合索引优化
  - 建议：为高频查询建立索引；合理设置过期策略与清理任务；监控内存/连接池
  - **新增**：检查点存储应考虑数据量增长，定期清理旧检查点

### 检查点存储优化
- **共享内存优化**：当检查点存储与共享内存存储相同时，避免重复存储共享内存条目
- **增量存储**：检查点只保存必要的状态信息，避免全量快照
- **键空间管理**：使用预留的检查点键空间，避免与共享内存冲突
- **最佳努力写入**：检查点写入失败不影响主运行，确保系统的稳定性

**章节来源**
- [packages/core/src/memory/checkpoint.ts:15-29](file://packages/core/src/memory/checkpoint.ts#L15-L29)
- [packages/core/tests/checkpoint.test.ts:333-366](file://packages/core/tests/checkpoint.test.ts#L333-L366)
- [packages/core/tests/checkpoint.test.ts:368-394](file://packages/core/tests/checkpoint.test.ts#L368-L394)

## 故障排查指南
- 构造 SharedMemory 时传入的对象不满足 MemoryStore 结构
  - 现象：抛出类型错误，提示缺少必要方法
  - 处理：确保实现 get/set/list/delete/clear；如需 TTL，实现 setWithExpiry
- 写入 TTL 后未过期
  - 现象：后端未实现 setWithExpiry，SharedMemory 降级为普通写入
  - 处理：实现 setWithExpiry 或在应用层主动删除
- 读取到空值
  - 可能原因：键不存在、已被过期过滤、后端删除
  - 处理：确认命名空间格式与 agentName/key 拼接；检查过期回合数
- 摘要为空
  - 可能原因：store 为空或全部过期
  - 处理：确认写入流程与过期逻辑
- **新增**：检查点相关问题
  - 检查点写入失败：检查点写入失败不会中断运行，但会通过 onProgress 事件报告
  - 恢复失败：检查点损坏或格式不正确会导致恢复失败，需要手动清理检查点键
  - 存储空间不足：检查点存储空间不足会影响检查点写入，需要清理旧检查点或扩容存储

**章节来源**
- [src/memory/shared.ts:77-85](file://src/memory/shared.ts#L77-L85)
- [src/memory/shared.ts:168-184](file://src/memory/shared.ts#L168-L184)
- [src/memory/shared.ts:200-205](file://src/memory/shared.ts#L200-L205)
- [packages/core/src/memory/checkpoint.ts:47-89](file://packages/core/src/memory/checkpoint.ts#L47-L89)
- [packages/core/tests/checkpoint.test.ts:425-457](file://packages/core/tests/checkpoint.test.ts#L425-L457)

## 结论
Open Multi-Agent 的内存管理以 MemoryStore 抽象为核心，通过 SharedMemory 提供团队级共享与 TTL 能力。新增的检查点恢复系统进一步增强了框架的可靠性，支持长期任务的工作流持久化和崩溃恢复。默认 InMemoryStore 适合开发与测试，生产环境推荐持久化与跨进程共享的后端（如 Engram、Redis、PostgreSQL）。实现自定义 MemoryStore 时，务必满足接口契约并考虑 TTL、清理与可观测性。检查点系统提供了灵活的持久化选项，开发者可以根据应用场景选择合适的存储方案。

## 附录：配置与集成示例

### 启用默认共享内存（单进程）
- 在团队配置中设置 sharedMemory=true 即可启用默认 InMemoryStore

**章节来源**
- [docs/shared-memory.md:3-11](file://docs/shared-memory.md#L3-L11)

### 注入自定义 MemoryStore（示例：Engram）
- 创建 EngramMemoryStore 实例并传入团队配置的 sharedMemoryStore
- 示例脚本展示了如何在 runTeam 中注入 EngramMemoryStore 并运行研究流水线

**章节来源**
- [docs/shared-memory.md:13-25](file://docs/shared-memory.md#L13-L25)
- [examples/integrations/with-engram/team-research.ts:178-198](file://examples/integrations/with-engram/team-research.ts#L178-L198)

### 检查点配置示例
- **启用检查点**：在 runTasks 调用中传递 `{ checkpoint: { store } }`
- **全局启用**：通过 OrchestratorConfig 设置默认检查点配置
- **使用 runId**：当团队没有共享内存存储时，使用 `runId` 参数区分不同的运行
- **恢复运行**：使用 `restore()` 方法从检查点恢复

**章节来源**
- [docs/checkpoint.md:11-32](file://docs/checkpoint.md#L11-L32)
- [docs/checkpoint.md:45-78](file://docs/checkpoint.md#L45-L78)
- [packages/core/tests/checkpoint.test.ts:196-211](file://packages/core/tests/checkpoint.test.ts#L196-L211)

### 自定义 MemoryStore 开发清单
- 实现 get/set/list/delete/clear
- 如需 TTL，实现 setWithExpiry
- keys 为字符串；values 为字符串（结构化数据需序列化）
- 元数据透传，不修改调用方提供的字段
- 注意并发安全与过期过滤时机（读取时过滤，删除由后端负责）

**章节来源**
- [src/types.ts:992-1011](file://src/types.ts#L992-L1011)
- [src/memory/shared.ts:177-183](file://src/memory/shared.ts#L177-L183)

### 检查点最佳实践
- **存储分离**：检查点存储与共享内存存储分离，避免竞争和数据冗余
- **键空间管理**：使用预留的检查点键空间，确保与共享内存不冲突
- **定期清理**：定期清理旧的检查点，避免存储空间无限增长
- **监控告警**：监控检查点写入失败率，及时发现存储问题
- **备份策略**：重要运行的检查点应有备份策略，防止意外丢失

**章节来源**
- [packages/core/src/memory/checkpoint.ts:15-29](file://packages/core/src/memory/checkpoint.ts#L15-L29)
- [packages/core/tests/checkpoint.test.ts:425-457](file://packages/core/tests/checkpoint.test.ts#L425-L457)

### 性能优化建议
- **共享内存优化**：当检查点存储与共享内存存储相同时，享受免重复存储的优势
- **异步写入**：检查点写入采用异步方式，不影响主运行性能
- **批量操作**：对于大量检查点操作，考虑批量处理减少存储压力
- **缓存策略**：合理设置检查点的缓存策略，平衡内存使用和恢复速度

**章节来源**
- [packages/core/tests/checkpoint.test.ts:333-366](file://packages/core/tests/checkpoint.test.ts#L333-L366)
- [packages/core/tests/checkpoint.test.ts:368-394](file://packages/core/tests/checkpoint.test.ts#L368-L394)